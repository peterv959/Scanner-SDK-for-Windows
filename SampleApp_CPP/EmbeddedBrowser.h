...
/***
	Class :		 CEmbeddedBrowser
	Description: Provides IDispatch support to the MSHTML interface for the Web browser constrol.
	Author:		 VRQW74
***/
class CEmbeddedBrowser : public CWnd
{
protected:
	DECLARE_DYNCREATE(CEmbeddedBrowser)
	CStringW	m_xmlPath;
...
public:
	...
	CEmbeddedBrowser()
	{
		...
		GetTempPathW( 1024, PathBuffer);
		m_xmlPath.Format(L"%s%d.xml", PathBuffer, GetCurrentProcessId());
		CFile xmlfile(m_xmlPath, CFile::modeCreate | CFile::modeWrite);
		xmlfile.Close();
	}

	~CEmbeddedBrowser()
	{
		if(m_xmlPath.GetLength() > 0)
			DeleteFileW(m_xmlPath.GetString());
	}
	...
	void ShowXML(BSTR& buffer)
	{
		...
		CFile xmlfile(m_xmlPath, CFile::modeCreate | CFile::modeWrite);
		xmlfile.Write(W2A(buffer), ::SysStringLen(buffer));
		xmlfile.Close();
		Sleep(100);
		Navigate(m_xmlPath, 0, 0, 0, 0);
	}

	void Clear()
	{
		Navigate(_T("about:blank"), 0, 0, 0, 0);
	}
